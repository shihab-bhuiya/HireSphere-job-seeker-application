...
export default function CompanyProfile({ recruiter, recruiterCompany }) {
  // 1. Core State Initialization with explicit support for database array formats
  const initialData = Array.isArray(recruiterCompany) ? recruiterCompany[0] : recruiterCompany;
  ...
  const handleSubmit = async (e) => {
    ...
    const newCompanyData = {
      ...
      // FIXED: Fallback to existing status if updating, otherwise default to "active" to unlock job creation
    ...
    try {
      ...
      if (payload?.insertedId) {
        ...
        setErrors({});
        setIsEditing(false);
      } else if (payload?.acknowledged || payload?.modifiedCount) {
        setCompany({
          ...newCompanyData,
          _id: company?._id
        });
        ...
        toast.error("Profile could not be saved. Check backend actions.");
      }
    } catch (err) {
