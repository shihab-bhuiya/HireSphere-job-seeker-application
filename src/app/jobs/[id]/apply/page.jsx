...
const ApplyPage = async ({ params }) => {
    ...
    console.log('Plans of plan', plan);
    const job = await getJobById(id);

    const applicationCount = applications?.length || 0;

    // PERFECT MATCH: Using maxApplicationsPerMonth across all logic blocks
            ...
            <div className="max-w-3xl mx-auto space-y-8">
                ...
                <div className="bg-zinc-900 border border-zinc-800 rounded-2xl p-6 shadow-lg">
                    <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                        <div>
                            <span className="text-xs font-semibold uppercase tracking-wider text-zinc-500"> Monthly Quota Status
                            </span>
                            <h2 className="text-lg font-bold text-zinc-100 mt-0.5">  You have applied to <span className="text-blue-400">{applicationCount}</span> out of <span className="text-blue-400">{plan.maxApplicationsPerMonth}</span> positions
                            </h2>
                        </div>
